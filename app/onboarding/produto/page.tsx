...
export default function OnboardingProdutoPage() {
  ...
  useEffect(() => {
    ...
    getOnboardingCounts()
      .then(({ productCount, sellerCount }) => {
        ...
        else if (productCount > 0) router.replace('/onboarding/vendedor')
        else setReady(true)
      })
      .catch(() => setReady(true))
  ...
  async function handleCreate(values: { name: string; price: number; category: string; stock: number }) {
    try {
      ...
      router.replace('/onboarding/vendedor')
    } catch (err) {
      toast({
  ...
  return (
    <Card className="p-8 space-y-6">
      <div className="space-y-2">
        <p className="text-sm font-medium text-primary">Passo 1 de 2</p>
        <div className="inline-flex items-center justify-center w-11 h-11 rounded-full bg-primary/10 text-primary">
          <Package className="w-5 h-5" />
        </div>
        <h1 className="text-2xl font-bold text-foreground">Cadastre seu primeiro produto</h1>
        <p className="text-muted-foreground">
          Vamos começar pelo catálogo. Você poderá adicionar mais produtos depois.
        </p>
      </div>
