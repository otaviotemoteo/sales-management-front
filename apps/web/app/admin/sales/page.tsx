...
import { SalesStats } from '@/components/admin/sales/vendas-stats'
import { SalesFilters } from '@/components/admin/sales/vendas-filters'
import { SalesTable, type SaleRow } from '@/components/admin/sales/vendas-table'
import { SaleEditDialog } from '@/components/admin/sales/sale-edit-dialog'
import { SaleDetailModal } from '@/components/admin/sales/sale-detail-modal'
...
function mapSaleToRow(sale: SaleResponse): SaleRow {
  return {
    ...
    seller: sale.seller?.name?.split(' ')[0] ?? 'Seller',
...
export default function AdminSalesPage() {
  ...
  const MOCK_TO_API_STATUS: Record<Status, SaleStatus> = {
    completed: 'CONFIRMED',
    pending: 'PENDING',
    cancelled: 'CANCELLED',
  }
  ...
  async function handleSaveEdit(id: string, updates: { amount: number; status: Status }) {
    ...
    try {
      ...
      toast({ title: 'Could not update the sale', variant: 'destructive' })
    } finally {
      setEditOpen(false)
      setEditSale(null)
      refresh()
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Full view and control over every sale</p>
      </div>
